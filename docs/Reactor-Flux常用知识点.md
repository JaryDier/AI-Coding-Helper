...
三、人工介入的完整真实流程
我用一个最常见的 "报销审批" 场景给你演示：
用户输入："帮我报销 5000 元的差旅费"
执行思考节点：LLM 分析后决定调用「查询报销规则」工具
→ 思考完成 → 保存 Checkpoint 1
执行工具节点：调用「查询报销规则」工具，得到结果："5000 元以上需要部门经理审批"
→ 工具完成 → 保存 Checkpoint 2
执行思考节点：LLM 分析规则后，判断需要人工审批
→ 思考完成 → 保存 Checkpoint 3
→ 暂停执行，返回给用户："这笔报销需要部门经理审批，请确认"
等待人工输入：经理输入 "同意"
恢复执行：框架加载 Checkpoint 3
→ 把 "经理同意" 作为新的消息加入状态
→ 回到思考节点继续执行
执行思考节点：LLM 看到审批通过，决定调用「生成报销单」工具
→ 思考完成 → 保存 Checkpoint 4
执行工具节点：调用「生成报销单」工具
→ 工具完成 → 保存 Checkpoint 5
执行思考节点：LLM 生成最终结果
→ 思考完成 → 保存 Checkpoint 6
任务结束